		...
		<div class="board alert alert-secondary" role="alert">
			...
			<div class="row titleline">
				<div class="col-12">
					...
					<fmt:parseNumber value="${dto.write_date.time / (1000*60*60*24)}"
			...
			<div class="row subline">
				<div class="col-6 col-md-4">
					<i class="bi bi-pen"> ${dto.writer}</i>
				</div>
				<div class="col-6 col-md-8 right">
					<span><i class="bi bi-clock-history space">
			...
			<div class="row">
				<div class="col-12">
					<textarea id="contents" name="contents" disabled>${dto.contents}</textarea>
				</div>
			</div>
			...
			<c:if test="${not empty filelist}">
				<div class="card col-12">
					<div class="card-header">Download Files List</div>
					<ul class="list-group list-group-flush">

						<c:forEach var="i" items="${filelist}">
							<c:set var="num" value="${num+1}" />
							<li class="list-group-item"><i class="bi bi-link-45deg"></i>
								${num}. <a
								href="/file/download?sysname=${i.sysName }&oriname=${i.oriName }">
									${i.oriName }</a> <!-- <a href="/files/${i.sysName }"> ${i.oriName }</a> 이렇게하면 파일이 그냥 창에 열림-->
							</li>
							<!-- 다운로드 흉내 중 -->
						</c:forEach>
			...
			<c:choose>
				<c:when test="${not empty list}">
					<c:forEach var="i" items="${list}">
						<form action="/comment/modify" method="post" class="frmCm">
							...
							<div class="row commentedit">
								<div class="col-12 commentline">
									<span id="commentbtn"> <span id="commentwriter"><i
											class="bi bi-chat-square-quote"></i> ${i.writer}</span> <span
										id="commentdate"> ${i.formedDate}</span> <c:choose>
											<c:when test="${loginID == i.writer}">
												<button type="button"
													class="minibtn btn btn-outline-primary toCommentEdit"
													seq=${i.seq }>E</button>
												<button type="button"
													class="minibtn btn btn-outline-danger toCommentDelete"
													seq=${i.seq }>X</button>
											</c:when>
										</c:choose>
									</span>
								...
								<div class="col-12 autosizediv">
									<textarea class="commentsview" name="commentsAdd" disabled>${i.contents}</textarea>
								</div>
			...
			<form action="/board/toModify" method="post" id="frm">
				...
				<c:choose>
					<c:when test="${loginID != null}">
						...
						<div class="d-none" id="seq">
							...
							<input name="parent_seq" value="${dto.seq}"> <input
								name="seq" value="${dto.seq}">${dto.seq} <input
								name="title" value="${dto.title}"> <input
								name="contents" value="${dto.contents}">
						</div>
					...
					*/
							...
							if (result.isDenied) {
								Swal.fire('코멘트 삭제가 완료 되었습니다.', '', 'success')
									.then((result) => {
										...
										$(this).closest('form').attr("action", "/comment/delete?parent_seq="+${dto.seq}+"&seq=" + target);
					...
					$("#btncomment").on("click", function () {
						if($("#comments").val()==""){
							...
							$("#frm").attr("action", "/comment/write?parent_seq=${ dto.seq }")
							$("#frm").submit();
						}
					})
						...
						Swal.fire({
							...
							if (result.isDenied) {
								Swal.fire('게시글 삭제가 완료 되었습니다.', '', 'success')
									.then((result) => {
										$("#frm").attr("action", "/board/delete?seq=" + ${ dto.seq })
										$("#frm").submit();
									})
